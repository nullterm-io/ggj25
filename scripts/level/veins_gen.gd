...
func try_gen_positions() -> void:
	for i in range(0, _max_obstacles):
		...
		var j = sorted_positions.bsearch(gen_p)
		if j == 0:
			if _test_positions(Vector2(gen_p.x, gen_p.y), Vector2(sorted_positions[j].x, sorted_positions[j].y)):
				sorted_positions.insert(j, gen_p)
		elif j == sorted_positions.size() - 1:
			if _test_positions(Vector2(gen_p.x, gen_p.y), Vector2(sorted_positions[j - 1].x, sorted_positions[j - 1].y)):
				sorted_positions.insert(j, gen_p)
		elif sorted_positions.size() > 3 and sorted_positions.size() < _max_obstacles - 1:
			if _test_positions(Vector2(gen_p.x, gen_p.y), Vector2(sorted_positions[j - 1].x, sorted_positions[j - 1].y)) and \
				_test_positions(Vector2(gen_p.x, gen_p.y), Vector2(sorted_positions[j + 1].x, sorted_positions[j + 1].y)):
				sorted_positions.insert(j, gen_p)
		else:
			sorted_positions.insert(j, gen_p)
